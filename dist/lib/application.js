...
function Jayce() {
  ...
  this.actionCollection = function (actionsRouter) {
    this.actions = [].concat((0, _toConsumableArray3.default)(actionsRouter.actions));
  };
  ...
  this.action = function (type, callback) {
    ...
    if (typeof type === 'string' && typeof callback === 'function') {
      this.actions.push({ type: type, callback: callback });
    } else {
  ...
  ;this.listen = function (options) {
    ...
    ws.on('connection', function (con) {
      ...
      con.on('message', function (msg) {
        var ctx = messageParse.createContext(msg, con, that.clients); // 构建 事件上下文
        ...
        // 是否匹配到处理事件
        var isMatchAction = false;

        // 执行对应事件处理器
        that.actions.forEach(function (item, index) {
